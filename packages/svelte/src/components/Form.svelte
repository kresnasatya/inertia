<script lang="ts">
  ...
  let _method = $derived(typeof action === 'object' ? action.method : (method.toLowerCase() as FormComponentProps['method']))
  let _action = $derived(typeof action === 'object' ? action.url : action)
  ...
  let slotErrors = $derived($form.errors as Errors)
</script>
...
>
  {@render children?.({ errors: slotErrors, hasErrors: $form.hasErrors, processing: $form.processing, progress: $form.progress, wasSuccessful: $form.wasSuccessful, recentlySuccessful: $form.recentlySuccessful, clearErrors, resetAndClearErrors, setError, isDirty, submit, defaults, })}
</form>
